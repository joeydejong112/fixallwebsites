...
function handleDelete(scan: { _id: string }) {
  if (pendingDelete.value === scan._id) {
    emit('delete-scan', scan._id, new Event('click'))
    pendingDelete.value = null
    if (deleteTimer) { clearTimeout(deleteTimer); deleteTimer = null }
  } else {
    pendingDelete.value = scan._id
...
const filteredHistoryScans = computed(() => {
  ...
  return props.scans.filter((s: any) => {
    if (!s.url || !s.url.toLowerCase().includes(q)) return false
    ...
    // Note: pending/running scans are intentionally excluded from 'critical' filter
  ...
  <div>
    <div class="flex items-center gap-3">
      ...
      <input
        ...
        class="bg-[#0f0f14] border border-[#1e1e28] rounded-lg px-3 py-[7px] text-[13px] w-[260px] outline-none font-body transition-colors duration-150 placeholder:text-[#6b7280] focus:border-primary/40 text-[#e8e8f0]"
      ...
      <div class="flex gap-[2px] bg-white/3 border border-white/5 p-[3px] rounded-lg">
        <button
          ...
          :key="f"
          @click="filterStatus = f as 'all' | 'pass' | 'warning' | 'critical'"
          class="px-3 py-[5px] rounded-md text-[11px] font-semibold font-display uppercase tracking-[0.08em] bg-transparent border-none text-white/40 cursor-pointer transition-all duration-100 hover:bg-white/6"
    ...
    <div class="rounded-card bg-[#1e1e28]" style="padding:0;overflow:hidden;">
      <div v-if="!filteredHistoryScans.length" class="flex flex-col items-center justify-center" style="padding:40px 0;">
        ...
        <button v-if="scans.length" @click="filterStatus = 'all'; historySearch = ''" class="text-[12px] text-primary bg-transparent border-none cursor-pointer mt-1">Clear filters</button>
      </div>
      <div
        v-else
        ...
        role="button"
        tabindex="0"
        class="flex items-center gap-3 px-4 py-3 border-b border-white/[0.04] cursor-pointer w-full text-left text-inherit font-inherit last:border-b-0 hover:bg-white/[0.025]"
        @click="emit('open-scan', scan)"
        @keydown.enter.prevent="emit('open-scan', scan)"
        @keydown.space.prevent="emit('open-scan', scan)"
      >
        <div
          class="w-[7px] h-[7px] rounded-full flex-shrink-0"
          :class="{
            'bg-[#6b7280]': scan.status === 'pending',
            'bg-[#ffaa00]': scan.status === 'running',
            'bg-[#00d4aa]': scan.status === 'done',
            'bg-primary': scan.status === 'error',
          }"
        ></div>
        <div class="w-6 h-6 rounded flex items-center justify-center text-[11px] font-bold flex-shrink-0 text-[#9898b0] bg-white/[0.06] font-display">
        ...
        <div class="flex-1 min-w-0">
          <div class="text-[13px] text-[#e8e8f0] whitespace-nowrap overflow-hidden text-ellipsis font-body">{{ scan.url }}</div>
        ...
        <div class="font-display font-bold text-[20px] flex-shrink-0 w-10 text-right" :style="{ color: scan.status === 'done' ? scoreBg(scan.overallScore) : 'rgba(255,255,255,0.2)' }">
          {{ scan.status === 'done' ? (scan.overallScore ?? '—') : scan.status === 'running' ? '…' : '—' }}
        </div>
        <div class="flex gap-[2px] items-center flex-shrink-0" @click.stop>
          <button @click="emit('rescan', scan.url, $event)" class="w-7 h-7 rounded-md flex items-center justify-center bg-transparent border-none text-white/30 cursor-pointer transition-all duration-150 hover:bg-white/6 hover:text-white/70" title="Re-scan">
          ...
          <button @click="emit('toggle-monitor', scan.url, $event)" class="w-7 h-7 rounded-md flex items-center justify-center bg-transparent border-none text-white/30 cursor-pointer transition-all duration-150 hover:bg-white/6 hover:text-white/70" :class="{ 'text-primary': isMonitored(scan.url) }" title="Toggle monitor">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
          </button>
          <button @click="handleDelete(scan)" class="w-7 h-7 rounded-md flex items-center justify-center bg-transparent border-none text-white/30 cursor-pointer transition-all duration-150 hover:bg-primary/10 hover:text-primary" :class="{ '!text-primary animate-pulse': pendingDelete === scan._id }" :title="pendingDelete === scan._id ? 'Click again to confirm' : 'Delete'">
